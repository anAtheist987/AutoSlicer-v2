...
- **v2 最终成绩（正规协议：train 3 场 / val 1 场 / test 2 场，测试集全程未参与训练、
  选模与调参）**：**独立测试集事件级 F1 = 0.90**（45 首真值命中 40、误报 4，
  P 0.91 / R 0.89），帧级 F1 = 0.94。**3 折交叉验证（6 场金标留二法）事件级
  F1 = 0.93 ± 0.03**，帧级 0.956 ± 0.007；换随机种子重训事件级 F1 完全复现
  （0.904 vs 0.904）——训练稳定，折间差异来自场次多样性。
- v2 = 全频带 16kHz log-mel + 四类 powerset 输出头（无人声/本人唱/他人唱/纯伴奏
  + not-A 弱标签项）+ 带宽增强；v1 初版（8kHz 二分类）同协议下事件级 F1 0.83。
- 端到端 CLI 支持直接输入 B 站链接（src/bilibili.py 浏览器模拟下载 → 检测 → 切片）。
...
### 5.6 v2 正规协议复核（交叉验证 + 独立测试集，06-12 上午补做）

针对"是否过拟合/数据是否充足/数字是否被调参污染"三问的完整复核：

**3 折交叉验证**（6 场金标 VOD 留二法，每折独立重训 8000 步）：

| 折 | 验证 VOD | frame-F1 | event-F1 |
|---|---|---|---|
| A | 1188184012, 1190452750 | 0.965 | 0.904 |
| B | 1187358909_nb3, 1194102386 | 0.953 | 0.916 |
| C | 1211660979, 582716341 | 0.950 | 0.966 |
| **mean ± std** | | **0.956 ± 0.007** | **0.929 ± 0.031** |

折 A 换种子（42→123）重训：event-F1 0.904 vs 0.904 —— 种子方差≈0，
折间 ±3pt 全部来自"哪场 VOD 当验证"（场次多样性是当前瓶颈，不是过拟合：
训练集 VOD 上 frame-F1 0.97 vs 验证 0.96，差距可忽略；训练 loss 收敛在
0.07–0.13 未塌零）。

**独立测试集终评**（train={1188184012, 1187358909_nb3, 1194102386}，
val={1190452750} 仅用于选 checkpoint 与调后处理参数（on=0.5/off=0.3，其余默认），
test={1211660979, 582716341} 全程未参与任何决策，只评一次）：

| | frame-F1 | event-F1 | P / R | 事件 | 切点 MAE |
|---|---|---|---|---|---|
| test 合计 | 0.940 | **0.899** | 0.909 / 0.889 | 40/45 命中, 4 误报 | onset 13.5s / offset 12.7s |
| 1211660979 | | 0.923 | | 20 真值 | |
| 582716341 | | 0.880 | | 25 真值 | |

结论：未污染的测试数字（0.90）与 CV 均值（0.93±0.03）一致；终评模型训练集比 CV
少一场金标 VOD，0.90 可视为保守下界。生产模型可在全部 6 场上重训（预期≈CV 均值）。

### 5.7 生产模型与切点精修调查（协议复核后的后续改进）

**生产模型 v2_prod（release v2.1.0）**：协议验证完成后，按"CV 定超参→全量重训"的
标准做法在 5 场金标上重训（留 1188184012 选 checkpoint，其上 19/19 全对）。
性能引用 CV 估计 0.929±0.031；歌手区分回归（他人演唱 MV）0 误切。

**切点精修：调查后放弃默认启用**。误差分解显示 onset/offset **中位误差仅 ±1s**，
MAE ~13s 由少数离群段贡献。逐例诊断离群段（分离 stem 特征复核）：约半数是
**真在唱但人工未标**的歌前练唱/跟哼（人声+伴奏+持续音高都在），其余为说话误触发
与间隙填充桥接。即后处理层面已无干净的改进空间——强行用更紧的边界阈值回收
（已实现为 `PostProcessConfig.refine_edges`，默认关）在三折上 F1 持平、MAE 仅
-0.5s 且部分折 p90 恶化。切点尾部精度受标注口径限制，进一步提升需要更细的
边界标注或 onset/offset 回归头。
